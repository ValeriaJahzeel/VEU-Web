...
def mostrar_filtro_cp():
    ...
    # Guardar el CP en session_state para mantenerlo entre renderizados
    if buscar and codigo_postal:
        st.session_state.filtro_cp = codigo_postal
...
def mostrar_mapa_filtrado(codigo_postal=None):
    ...
    if codigo_postal:
        ...
        # Tratar de obtener información del CP del archivo
        coordenadas_cp = obtener_coordenadas_cp(codigo_postal)
        
        # Si hay reportes o coordenadas del CP, crear mapa centrado
        if reportes_filtrados or coordenadas_cp:
            try:
